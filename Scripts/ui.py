...
def run_gpt():
    ...
    prompt = entry_gpt_prompt.get("1.0", tk.END).strip()
    ...
    # Set the GPT prompt
    gpt_prompt = config["mistral"]["prompt"]

    try:
        os.chdir("GPT")  # Change directory to GPT folder
        config["mistral"]["output_file"] = output_file  # Update the output file path
        config["mistral"]["output_file"] = output_file
        config["mistral"]["prompt"] = prompt
        with open("../config.json", "w") as config_file:
            json.dump(config, config_file, indent=4)
        subprocess.run(["python", "GPTAPI.py"])
        messagebox.showinfo("Mistral", "Mistral script completed successfully.")
        os.chdir("..")  # Change back to the previous directory
...
label_gpt_settings = tk.Label(frame_gpt_settings, text="Mistral Settings", font=(font_family, 14, "bold"), bg=background_color)
...
label_gpt_api_key = tk.Label(frame_gpt_api_key, text="Mistral API Key:", font=(font_family, font_size), bg=background_color, fg=text_color)
...
label_gpt_prompt = tk.Label(frame_gpt_prompt, text="Mistral Prompt:", font=(font_family, font_size), bg=background_color, fg=text_color)
...
entry_gpt_prompt.insert(tk.END, config["mistral"]["prompt"])
...
entry_output_file.insert(tk.END, config["mistral"]["output_file"])
...
def save_settings():
    ...
    config["mistral"]["prompt"] = entry_gpt_prompt.get("1.0", tk.END)
    config["mistral"]["output_file"] = entry_output_file.get()
...
button_gpt = tk.Button(window, text="Run Mistral", command=run_gpt)
button_gpt.pack()
